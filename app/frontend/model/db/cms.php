 ...
 * @version    2.0
 * @id $Rev$
 * @author Gérits Aurélien <aurelien@magix-cms.com> <support@magix-cms.com>
 * @name cms
 *
 */
class frontend_model_db_cms
{
    /**
	...
	protected function s_page_data($idpage)
    {
		...
		return component_routing_db::layer()->fetch($select,array(
			':idpage'=>$idpage
		));
	}
    ...
    protected function s_page($lang_iso,$sort_id=null,$sort_type=null,$limit=null)
    {
    	...
    	ORDER BY p.order_page
    	  {$limit_clause}";
        return component_routing_db::layer()->fetchAll($select,array(
            ':lang_iso' => $lang_iso
        ));
    }
