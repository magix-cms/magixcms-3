var gmap = (function ($, undefined) {
    ...
    function init(config,options) {
        ...
        self.defaults = {
			...
			zoom: 15,
			mapTypeId: "lodlys"/*google.maps.MapTypeId.ROADMAP*/,
			mapTypeControl: true,
			mapTypeControlOptions: {
				mapTypeIds: [google.maps.MapTypeId.ROADMAP, "lodlys"],
        ...
        if(config.markers.length > 1) {
			...
			$.each(config.markers, function(key, val) {
				...
				self.markers.push({
					position: latLng,
					icon: {
						url: "/gmap/?marker=main&dotless=true",
    ...
    function loadMap(options) {
        ...
        self.gMap.styledmaptype(
            "lodlys",
            [
                {"elementType": "geometry", "stylers": [{"color": "#f5f5f5"}]},
                {"elementType": "labels.icon", "stylers": [{"saturation": -100}, {"visibility": "simplified"}]},
				{"elementType": "labels.text.fill", "stylers": [{"color": "#808284"}]},
				{"elementType": "labels.text.stroke", "stylers": [{"color": "#f5f5f5"}]},
                {"featureType": "administrative.land_parcel", "elementType": "labels.text.fill", "stylers": [{"color": "#bdbdbd"}]},
                {"featureType": "poi", "elementType": "geometry", "stylers": [{"color": "#eeeeee"}]},
                {"featureType": "poi", "elementType": "labels.text.fill", "stylers": [{"color": "#808284"}]},
                            ...
                            self.flags = [
                            	{
									...
									icon: {
										url: "/gmap/?marker=grey&dotless=true",
    ...
    function getDirection() {
		...
		$('.hidepanel').on('click',function(){
			...
			else if(icon.text() === 'keyboard_arrow_right') {
				bloc.css('left', '0');
            ...
            else if(icon.text() === 'keyboard_arrow_down') {
                bloc.css('top', '0');
		...
		$('#showform').on('click',function(){
			var icon = $('#showform > .material-icons');
			if(icon.text() === 'directions') {
				icon.text('close');
			}
			else {
