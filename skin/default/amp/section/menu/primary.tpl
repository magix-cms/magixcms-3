...
{/if}
{/strip}
<div id="header-menu" class="affix">
    <nav id="main-menu" class="menu{if $mobile} collapse navbar-collapse{/if} menu-tabs-arrow menubar" itemprop="hasPart" itemscope itemtype="http://schema.org/SiteNavigationElement">
        {if !$mobile}<div class="container">{/if}
            <a href="#content" class="sr-only skip-menu">{#skipMenu#}</a>
            <ul class="nav">
                {include file="section/menu/loop/tabs-arrow.tpl" menuData=$menu mobile=$mobile}
            </ul>
        {if !$mobile}</div>{/if}
    </nav>
</div>