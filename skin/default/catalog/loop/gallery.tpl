<div id="gallery">
    <div class="image-gallery">
        ...
        <div class="owl-carousel owl-theme-square thumbs">
            {foreach $product.imgs as $k => $item}
                <a class="show-img" href="#" data-target="#{if $item.default}default{else}img{$k}{/if}" rel="productGallery"><img {*src="{$product.img_default}"*} class="img-responsive owl-lazy" data-src="{$item.img.small.src}" alt="{$item.img.alt}"{if $item.img.small.crop === 'adaptative'} width="{$item.img.small.w}" height="{$item.img.small.h}"{/if}/></a>
            {/foreach}
        </div>
    </div>
</div>