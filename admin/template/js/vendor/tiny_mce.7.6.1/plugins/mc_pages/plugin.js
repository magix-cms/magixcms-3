...
tinymce.PluginManager.add('mc_pages', function(editor, url) {
    ...
    // Icône : On utilise baseadmin s'il est défini, sinon un fallback
    const iconPath = (typeof baseadmin !== 'undefined')
        ? '/' + baseadmin + '/template/img/tinymce/mc_icons.svg#page'
        : url + '/img/mc_icons.svg#page';

    editor.ui.registry.addIcon('page', `<svg width="24" height="24"><use xlink:href="${iconPath}"></use></svg>`);

    /* Ouverture du dialogue */
    const showDialog = () => {
        editor.windowManager.openUrl({
            title: _('mc_pages Title'), // Traduction centralisée
            url: tinymce.baseURL + '/plugins/mc_pages/pages.php',
            width: 800,
            height: 550
        });
    };
    ...
    // Ajout au menu Insertion
    editor.ui.registry.addMenuItem('mc_pages', {
        icon: 'page',
        text: _('mc_pages Title'),
        onAction: showDialog
    });
