...
<div class="row">
    <form id="edit_cat" action="{$smarty.server.SCRIPT_NAME}?controller={$smarty.get.controller}&amp;action=edit&edit={$page.id_cat}" method="post" class="validate_form edit_form_extend col-ph-12 col-md-10">
            ...
            {foreach $langs as $id => $iso}
                <fieldset role="tabpanel" class="tab-pane{if $iso@first} active{/if}" id="lang-{$id}">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="content[{$id}][content_cat]" id="content[{$id}][content_cat]"
                                  class="form-control mceEditor"
                                  data-controller="{$smarty.get.controller}"
                                  data-itemid="{$smarty.get.edit}"
                                  data-lang="{$id}"
                                  data-field="content_cat">{call name=cleantextarea field=$page.content[{$id}].content_cat}</textarea>
                    </div>
                    ...
                    <div id="link-{$id}" class="collapse">
                        <div class="row">
                            <div class="col-ph-12 col-sm-8">
                                <div class="form-group">
                                    <label for="content[{$id}][link_label_cat]">{#custom_link_label#} :</label>
                                    <textarea class="form-control" id="content[{$id}][link_label_cat]" name="content[{$id}][link_label_cat]" cols="70" rows="3">{$page.content[{$id}].link_label_cat}</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-ph-12 col-sm-8">
                                <div class="form-group">
                                    <label for="content[{$id}][link_title_cat]">{#custom_link_title#} :</label>
                                    <textarea class="form-control" id="content[{$id}][link_title_cat]" name="content[{$id}][link_title_cat]" cols="70" rows="3">{$page.content[{$id}].link_title_cat}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
