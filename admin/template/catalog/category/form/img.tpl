<div class="row">
    ...
    {/foreach}
    <form id="edit_img_cat" action="{$smarty.server.SCRIPT_NAME}?controller={$smarty.get.controller}&amp;action=edit&amp;edit={$page.id_cat}" method="post" class="validate_form edit_form_img col-ph-12 col-md-6 col-lg-5">
        <h3>Image, description de l'image et SEO</h3>
        <div class="form-group">
            <label for="name_img_{$id}">{#name_img#|ucfirst} :</label>
            <input type="text" class="form-control" placeholder="{#ph_name_img#}" id="name_img_{$id}" name="name_img" value="{if isset($page.img_cat)}{$page.img_cat}{else}{$page.content[$default].url_cat}{/if}" />
        </div>
        ...
        <fieldset>
            ...
            {if $edit}
                <input type="hidden" name="page[id]" value="{$page.id_cat}" />
            {/if}
