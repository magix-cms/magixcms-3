{if isset($data) && !empty($data)}
    ...
    {foreach $data as $key => $value}
        <div id="images_{$value.id_img}" class="col-ph-12 col-xs-6 col-sm-4 col-md-3 col-lg-2">
            <input type="checkbox" name="img[{$value.id_img}]" id="{$value.name_img}" value="{$value.id_img}">
            <label for="{$value.name_img}">
                <figure>
                    <div class="center-img">
                        <img class="img-responsive" src="/upload/{if isset($uploadDir)}{$uploadDir}{else}{$smarty.get.controller}{/if}/{$value.id_news}/s_{$value.name_img}" />
                    </div>
                </figure>
            </label>
            ...
            <div class="make-default{if $value.default_img} hide{/if}">
                <a href="#" class="make_default" data-id="{$value.id_img}" data-edit="{$value.id_news}">
